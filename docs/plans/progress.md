...
| H | Phase 2 — in-app skill runner on the Foundry deployment (spec [`phase2h`](./foundry-codex-migration-phase2h.md)) | **H-a + H-b done** (`8c06716`, `a3f7e76`); H-c (IPC + Skills UI) in progress | GH — ② live smoke **PASSED**; ③ manual UI run pending (user) | tests 310/310; live 2026-08-01: fixture skill on `gpt-5.3-codex` resolved Contoso via `api:listCustomers`, confirmation-gated `api:createSalesOrder` → HTTP 201, order SO-10003 verified in the testbed API state; credentials redacted (zero `demo-key-123` occurrences in the transcript) |
